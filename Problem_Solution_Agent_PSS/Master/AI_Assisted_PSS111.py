...
def _build_engineering_prompt(user_text: str, supply_chain_scenario: bool = False) -> str:
    ...
    if supply_chain_scenario:
        sc_rules = (
            ...
            "  - Any specific demand quantity or demand figure (e.g., 'demand of 60', '120 units demand')\n"
            "  - Any PO / Purchase Order number (numeric sequences associated with purchase orders)\n"
            "  - Any quarter or date coverage reference (e.g., '25Q3', 'Q4 2025', 'through Q3', 'beyond Q4')\n"
            "  - Any supply status timestamp (e.g., 'as of 09 Apr 2025')\n"
            "  - Any numeric count linked to inventory, demand, or order volume\n\n"
            "CRITICAL: Even if the input contains these items, your output MUST NEVER reproduce them.\n"
            "They are internal supply data and do NOT belong in engineering documentation.\n\n"
            "MANDATORY for Problem Description:\n"
            "  - Select 3 to 5 approved generic statements below that best match the supply risk.\n"
            "  - Pair each selected statement with the affected part number(s) from the input.\n"
            "  - Do NOT invent new phrasing for inventory or demand status; use only the text below.\n\n"
            "APPROVED GENERIC STATEMENTS (select and use verbatim, paired with part number):\n"
            + _SC_GENERIC_STATEMENTS
            ...
            "    cross-functional escalation, supplier engagement, expedite supply review.\n"
        ...
        sc_input_warning = (
            ...
            "generic supply chain criticality statements provided in the rules above.\n\n"
        )
    return f"""
...
{_PROBLEM_CATEGORY_TAXONOMY}

TITLE:
- MANDATORY: Use the sub-category label from the Step 3 lookup table above as the core of the title.
- Format: <PART_NUMBER> - <Sub-Category Label from Step 3 table>
- Example: if sub-category is "Missing Tolerance on DWG", title = "NNNN-NNNNN - Missing Tolerance on DWG"
- Example: if sub-category is "OEM EOL Declaration - Replacement P/N Not Yet Identified", title = "NNNN-NNNNN - OEM EOL Declaration - Replacement P/N Not Yet Identified"
- Copy the sub-category label verbatim from the Step 3 table. Do NOT paraphrase it.
- Short and technical.
...
def reframe_problem(user_text: str, supply_chain_scenario: bool = False) -> str:
    ...
    sc_fix_rule = (
        "\n- SUPPLY CHAIN SHORTAGE SCENARIO: Do NOT include inventory quantities,"
        " demand quantities, or PO numbers. Use only generic supply chain criticality"
        " language paired with the affected part number(s) in Problem Description.\n"
    ) if supply_chain_scenario else ""

    violations = validate_engineering_output(draft, user_text)
    # Only trigger the expensive fix-pass for CRITICAL violations:
    # missing required sections or SPS# appearing in the output.
    _sps_in_output = bool(re.search(r'\bSPS\s*#?\s*\d', draft, re.IGNORECASE))
    _missing_sections = any(
        v for v in violations
        if any(kw in v for kw in ("Missing 'Title'", "Missing 'Problem", "Missing 'Solution", "must not be empty"))
    )
    critical_violations = [v for v in violations if (
        _sps_in_output or _missing_sections or
        any(kw in v for kw in ("SPS", "Missing", "must not be empty"))
    )]
...
Keep exactly:
Title
Problem Statement (Change Drivers when applicable, Affected/Impacted Part Numbers OR Affected Part Numbers + Issue Part Number(s), Problem Description, Impact Details)
Solution Statement (Solution Description, Benefits of the Proposed Solution)

Important:
- If reference records exist in the input, include "Change Drivers:" before the part number subsections.
- Read ALL PCR and Project records together and write exactly 2 combined bullets starting with "This change is to drive" — complete sentence, max 15 words each. Do NOT write separate bullets per PCR or Project.
- Determine Primary Issue/Problem source in priority order: SPS, then QN, then PCR.
- Keep Title/Problem/Solution anchored to that primary source; use PCR/Project only as high-level supporting context.
- For ESW records in Change Drivers: one line per ESW summarising WHAT the ESW is doing (not its title). Do NOT include an SPS line in Change Drivers.
- ESW Turn Around is high-level interim context only, using 1 to 2 concise bullets.
- If SPS data is present, translate its content into professional third-person language in Problem Description WITHOUT citing any SPS number.
- SPS# PROHIBITION (MANDATORY): Do NOT write any SPS number anywhere in the output — not in Change Drivers, Problem Description, Solution Statement, or any other section.
...
- STRICT SECTION SEPARATION -- do NOT mix Problem and Solution content:
    ...
    - Solution Description: SOURCE-BOUND ONLY. Use imperative action language. OMS DOMAIN RULE: any part number starting with '0251-' is an OMS document -- always refer to it as 'OMS <part-number>'. GROUPING RULE (MANDATORY): ONE CORRECTIVE ACTION = ONE TOP-LEVEL BULLET ONLY. If one action applies to multiple OMS documents, write the action once and list each OMS document as an indented sub-bullet with page numbers if available. SPS# PROHIBITION (MANDATORY): SPS# must NEVER appear anywhere in the Solution Statement. Do NOT repeat the action sentence per document.
...
- SENTENCE LENGTH — PROBLEM DESCRIPTION (MANDATORY): Each Problem Description bullet must be maximum 15 words. If a bullet exceeds 15 words, split into sub-bullets or compress.
- CONTEXT ANCHORING IN PROBLEM DESCRIPTION (MANDATORY): The first bullet must establish context (Assy DWG + part number) in one sentence; all remaining bullets must NOT repeat those references. Do NOT cite any SPS number in any bullet.
- If SPS provides issue part number(s), focus Problem Description bullets on issue part details.
- Always include supplier name in Problem Description.
- Mention reporting parent/where-used chain in only one bullet; do not repeat parent hierarchy in multiple bullets.
- If obsolete/EOL is present, write that Issue P/N is declared obsolete, not parent P/N.
- Do NOT include identified replacement part number details in Problem Description; keep those in Solution Description only.
- Keep only first-level parent relation and place parent-structure bullet as the 2nd bullet.
- When supplier MPN exists, format as Applied <Commodity Class> P/N first with supplier P/N in brackets.
- In obsolete/EOL cases, include first-level parent in the same obsolete bullet.
- Include commodity class label (for example, OEM) with Applied part number where applicable.
- PART NUMBERS ONLY — NO DESCRIPTIONS: Write part numbers alone. Do NOT append any part name, description, or parenthetical text after a part number in Problem Description or Solution Description. WRONG: '3310-00443 (2 INCH INDICATING PRESSURE TRANSMITTER)'. RIGHT: '3310-00443'.
...
- Title P/N must be based on Issue Part Number(s), not Affected parent part number(s).
- If exactly one Issue Part Number exists, use that Issue P/N in title prefix.
- If more than one Issue Part Number exists, do not include issue P/N prefix in title.
...
- If detected BOM Parent/Child or obsolete/replacement pairs are present in input, use them explicitly and consistently in both Problem and Solution sentences.{sc_fix_rule}
- Obsolescence solution pattern:
    - If replacement is identified: include replacement Applied P/N and Supplier P/N in brackets; include qualification for FFF; then parent interchangeability/FFF decision.
    - If replacement is not identified: identify and qualify alternate part/supplier meeting FFF, perform validation/testing, and update BOM/Drawings/Specifications/Work instructions.{sc_fix_rule}
Input:
<<<
{user_text}
>>>
...
def _reframe_long_solution_bullets(text: str, max_words: int = 15) -> str:
    ...
    prompt = (
        ...
        "2. Use abbreviations where allowed: DWG, BOM, P/N, Assy, Spec, Qty, Rev.\n"
...
6. Use ONLY the approved generic and scenario-specific statements below — verbatim or closely paraphrased.
...
APPROVED GENERIC SUPPLY CHAIN CRITICALITY STATEMENTS — USE 3 TO 5 IN PROBLEM DESCRIPTION:
{_SC_GENERIC_STATEMENTS}
...
APPROVED SOLUTION ACTIONS — USE 2 TO 4 IN SOLUTION DESCRIPTION:
  ...
  - Conduct supply chain risk assessment and define a contingency buffer strategy.
