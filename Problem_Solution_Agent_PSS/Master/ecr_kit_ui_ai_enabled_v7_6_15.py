...
class InventoryCostTab(QWidget):
    ...
    def export_excel(self):
        ...
        if out_path.exists():
            try:
                ...
                QMessageBox.critical(self, 'File In Use', 'Close Excel file first')
                return

        with pd.ExcelWriter(out_path, engine='openpyxl') as w:
            # write dataframe so column headers land on row 3
            self.df.to_excel(w, index=False, sheet_name='Output Sheet', startrow=2)
        wb=load_workbook(out_path); ws=wb['Output Sheet']

        # row 1 = merged plant header (added below)
        # row 2 = delete
        ws.delete_rows(2)

        # ---- build merged header (row 1 only) ----
        col=6  # first plant column index
        for p in PLANTS:
            span=len(METRICS)
            ws.merge_cells(start_row=1,start_column=col,end_row=1,end_column=col+span-1)
            cell=ws.cell(1,col); cell.value=str(p)
            cell.font=Font(bold=True); cell.alignment=Alignment(horizontal='center')
            col+=span
        # totals header
        ws.merge_cells(start_row=1,start_column=col,end_row=1,end_column=col+5)
        ws.cell(1,col).value='Total'; ws.cell(1,col).font=Font(bold=True)
        ws.cell(1,col).alignment=Alignment(horizontal='center')

        # style
        header_fill=PatternFill('solid', fgColor='BDD7EE')
        cost_fill=PatternFill('solid', fgColor='FFF2CC')
        # ---- style + clean column headers (remove plant code text) ----
        for c in range(1, ws.max_column+1):
            cell = ws.cell(2, c)
            val = str(cell.value) if cell.value is not None else ''
            # remove leading plant code (e.g. "4020 On Order Qty" -> "On Order Qty")
            val = re.sub(r'^\d{4}\s+', '', val)
            cell.value = val
    ...
    def reset_tab(self):
        self.df=None; self.table.clear(); self.table.setRowCount(0); self.table.setColumnCount(0)
# ====================================================================
