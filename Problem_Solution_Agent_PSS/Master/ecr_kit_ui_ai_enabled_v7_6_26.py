...
class WURemovedBOMItemsTab(QWidget):
    ...
    def __init__(self, obs_provider, imp_bom_provider=None):
        ...
        # WU Level input beside the Import button
        wu_lbl = QLabel("WU Level (1-6):")
        self.wu_level_input = QLineEdit()
    ...
    def import_from_databricks_for_removed_items(self):
        """Read removed-child parts from Imp BOM, query Databricks WU in a background
        thread (so the UI stays responsive), then populate this table on the main thread."""
        ...
        # ── Read removed-child parts from Imp BOM tab ─────────────────────────
        if self.imp_bom_provider is None:
            QMessageBox.warning(self, 'Imp BOM Not Available',
                                'The Imp BOM tab is not connected. Cannot read removed child parts.')
            return
        ...
        for r in range(imp_table.rowCount()):
            ...
            if comment != 'removed child part':
                continue
            part_item = imp_table.item(r, part_col)
            if not part_item:
                continue
            raw = part_item.text().strip()
            if raw and raw.upper() not in seen:
                seen.add(raw.upper())
                removed_parts.append(raw)

        if not removed_parts:
            QMessageBox.information(self, 'No Removed Child Parts',
                                    'No rows with "Removed Child Part" in Tool comments were found in Imp BOM.\n'
                                    'Import a BOM file and ensure removed child parts are marked.')
            return
        ...
        # ── Import where_used_query module ────────────────────────────────────
        try:
            import sys as _sys
            from pathlib import Path as _Path
            _mod_dir = str(_Path(__file__).parent)
        ...
        class _Worker(QObject):
            finished = pyqtSignal(list)   # emits records list on success
            error    = pyqtSignal(str)    # emits error message string

            def __init__(self, parts, level, plant):
                super().__init__()
                self._parts = parts
                self._level = level
                self._plant = plant 
            ...
            def run(self):
                try:
                    # Use the parents-only variant: level-0 rows are synthesised
                    # locally (no Databricks round-trip), halving queries to 1
                    # for level 1. Same output structure as fetch_where_used().
        ...
        # ── Progress dialog (cancelable label, indeterminate) ─────────────────
        progress = QProgressDialog(
            f'Querying Databricks for {len(removed_parts)} part(s) up to WU level {max_level}…\n'
            'This may take a moment — please wait.',
            None,          # no Cancel button
            0, 0,          # min=max=0 → indeterminate spinner
            self,
        )
        progress.setWindowTitle('Importing Where Used…')
        progress.setMinimumDuration(0)
        progress.setModal(True)
        progress.show()

        # ── Thread setup ──────────────────────────────────────────────────────
        selected_plant = self.plant_combo.currentText()

        thread = QThread(self)
        worker = _Worker(removed_parts, max_level, selected_plant)
        worker.moveToThread(thread)

        # Capture locals for the slots below
        ...
        def _on_finished(records):
            ...
            if not records:
                QMessageBox.information(
                    self, 'No Data',
                    f'Databricks returned no results for {len(removed_parts)} removed-child part(s) '
                    f'at max WU level {max_level}.')
                return
            ...
            QMessageBox.information(
                self, 'Import Complete',
                f'Imported {len(records)} row(s) from Databricks.\n'
                f'Removed-child parts queried: {len(removed_parts)}  |  Max WU level: {max_level}'
            )
