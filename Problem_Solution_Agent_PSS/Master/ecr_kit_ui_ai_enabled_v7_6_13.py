...
class InventoryCostTab(QWidget):
    ...
    def __init__(self):
        ...
        for b in (self.btn_import, self.btn_export, self.btn_reset): h.addWidget(b)
        h.addStretch(1)
        v.addLayout(h)
    ...
    def import_mm360(self):
        ...
        if not path: return
        df = pd.read_excel(path, sheet_name='Material Analysis', engine='openpyxl')

        def pace(mp):
            mp = str(mp)
            if mp.startswith('SGP'): return 'PACE'
            if mp.startswith('GDS'): return 'DASH'
            return ''
        df['PACE/DASH'] = df['MRP Profile'].apply(pace)

        rows = []
        for (mn, desc, pdsh), g in df.groupby(['Material Number','Material Description','PACE/DASH']):
            row = {
                'Material Number': mn,
                'Material Description': desc,
                'Primary Disposition': g['Material Status'].iloc[0] if 'Material Status' in g else '',
                'Secondary Disposition': g['Cross Plant Status'].iloc[0] if 'Cross Plant Status' in g else '',
                'PACE/DASH': pdsh
            }
            total_cost = 0.0
            for p in self.plants:
                gp = g[g['Plant Code']==p]
                oo = float(gp['On Order Quantity'].sum()) if not gp.empty else 0.0
                oh = float(gp['onhand'].sum()) if not gp.empty else 0.0
                sc = float(gp['Standard Cost USD'].iloc[0]) if (not gp.empty and (oo or oh)) else 0.0
        ...
        self.df = pd.DataFrame(rows).sort_values('Total Cost', ascending=False)
        self.render()
    ...
    def render(self):
        ...
        for r in range(len(self.df)):
            for c,col in enumerate(self.df.columns):
                ...
                text = '' if (isinstance(v,(int,float)) and v==0) else str(v)
                self.table.setItem(r,c,QTableWidgetItem(text))
        self.table.resizeColumnsToContents()

    def export_excel(self):
        if self.df is None: return
        path,_ = QFileDialog.getSaveFileName(self,'Export','inventory_cost.xlsx','Excel Files (*.xlsx)')
        if not path: return
        with pd.ExcelWriter(path, engine='openpyxl') as w:
            self.df.to_excel(w, index=False, sheet_name='Output Sheet')
        # Excel formatting
        from openpyxl import load_workbook
        ...
        cost_fill = PatternFill('solid', fgColor='FFF2CC')
        for c in range(1, ws.max_column+1):
            ws.cell(1,c).font = Font(bold=True)
            ws.cell(1,c).fill = header_fill
            ws.cell(1,c).alignment = Alignment(horizontal='center')
            if 'Cost' in str(ws.cell(1,c).value):
                for r in range(2, ws.max_row+1):
                    ws.cell(r,c).number_format = '$#,##0.00'
                    ws.cell(r,c).fill = cost_fill
