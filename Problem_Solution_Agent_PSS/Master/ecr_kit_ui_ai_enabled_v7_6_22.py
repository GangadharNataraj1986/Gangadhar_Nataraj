...
class ECCreationInputsFormTab(QWidget):
    def __init__(self):
        super().__init__()
        self.email_file_path = None  # Store browsed email file path
...
# ---- Section B ----
    ...
    #    setContentsMargins(left, top, right, bottom)
        ...
        yes_no_with_box("PCR", "2.  Is there any Product Change Request (PCR)")
        ...
        def disable_psn():
                psn_yes.setAutoExclusive(False)
                psn_no.setAutoExclusive(False)
                psn_yes.setChecked(False)
                psn_no.setChecked(False)
                psn_yes.setAutoExclusive(True)
                psn_no.setAutoExclusive(True)
        ...
        browse = QPushButton("Browse E-mail")
...
# ...existing code...
    ...
    def on_problem_summary_clicked(self):
        try:
            ...
            payload = self._collect_pss_payload()
            
            # If email file is browsed, use email correlation
            if self.email_file_path:
                ...
                # Regular PSS generation without email
                out = self._run_ai_pss_full(payload)
    ...
    def _on_browse_email_clicked(self):
        """Handle email file browse and save the path."""
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Email", "", "Email Files (*.msg *.eml *.txt)"
        )
        if file_path:
            self.email_file_path = file_path
            QMessageBox.information(self, "Email Loaded", f"Email file loaded:\n{Path(file_path).name}")

    def on_problem_summary_clicked_old(self):
