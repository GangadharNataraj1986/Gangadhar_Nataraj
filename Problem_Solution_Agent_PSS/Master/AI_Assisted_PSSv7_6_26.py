...
def _build_reference_change_summary_from_payload(payload: Dict[str, Any]) -> str:
    ...
    PCR + Project records are combined into ONE LLM-generated sentence.
    ...
    prompt = (
        ...
        "Read ALL of the following PCR and Project records together and write exactly ONE sentence "
        "that gives a combined high-level overview of what all these records are collectively driving.\n\n"
        "Rules:\n"
        "- The sentence must start with 'This change is to drive'.\n"
        "- Maximum 20 words total.\n"
        "- Combine all objectives into a single coherent sentence.\n"
        "- Use plain professional language. Do NOT use arrows, special symbols, or SPS numbers.\n"
        "- Output ONLY the single sentence. No bullets, no preamble, no explanation.\n\n"
        "INPUT RECORDS:\n" + combined_input
    )
    ...
    return "\n".join(chunk.splitlines()[0] for chunk in pcr_project_chunks)


def _build_esw_turnaround_from_payload(payload: Dict[str, Any]) -> str:
    """
    Builds the 'ESW Turn Around (ESW# XXXXX):' section.
    One header per ESW with 2 LLM-generated bullets describing the temporary workaround.
    ...
    for rec in esw_records:
        ...
        problem = _to_single_line_summary(rec.get("problem") or "")
        solution = _to_single_line_summary(rec.get("solution") or "")
        esw_input = f"Title: {title}\n  Problem: {problem}\n  Solution: {solution}"
        prompt = (
            "You are a senior manufacturing engineer.\n\n"
            "Read the following ESW record and write exactly 2 bullet points that briefly describe "
            ...
            "- Output ONLY the 2 bullet lines starting with '- '. No preamble, no explanation.\n\n"
            "ESW RECORD:\n" + esw_input
        )
        result = _call_databricks(prompt)
        ...
        if result and not result.startswith("Error:") and result.strip():
            ...
            bullet_text = f"- Temporary workaround implemented via ESW to address the issue.\n- Permanent fix to be incorporated through the ECR."
...
def _build_engineering_prompt(user_text: str, supply_chain_scenario: bool = False) -> str:
    # Pre-select best SC statement once (used in sc_rules block).
    _sc_label, _sc_text = (
        _select_sc_generic_statement(user_text) if supply_chain_scenario else ("", "")
    )
    sc_rules = ""
    sc_input_warning = ""
    if supply_chain_scenario:
        ...
        sc_input_warning = (
            ...
            "generic supply chain criticality statement provided in the rules above.\n\n"
        )
    return f"""
...
- Exactly 2 bullets only. Each bullet must be maximum 15 words.
...
def _normalize_problem_statement(problem_section: str, source_text: str = "") -> str:
    ...
    esw_turnaround = _extract_subsection(problem_section, "ESW Turn Around", PROBLEM_SUBSECTION_NAMES)
...
def _rebalance_problem_solution(
    ...
    if ref:
        rebuilt_problem_blocks.append("Change Drivers:\n" + ref.strip())
    if aff_imp:
...
_RC_CLASSIFICATION_SYSTEM_PROMPT = (
    ...
    "---------------------------------------------------------------------\n"
    "DOCUMENT CORRECTION \u2013 4.5 (STRICT / LAST-RESORT CATEGORY)\n"
    "---------------------------------------------------------------------\n"
    "Use 4.5 ONLY when ALL conditions below are TRUE:\n\n"
    "\u2022 The item is already in Production release\n"
    "\u2022 The ECO corrects documentation errors only\n"
    "\u2022 NO change to:\n"
    "  \u2013 form, fit, function\n"
    "  \u2013 performance, quality, reliability, or safety\n"
    "  \u2013 manufacturability\n"
    "  \u2013 inspection methods\n"
    "  \u2013 acceptance criteria\n\n"
    "Allowed examples for 4.5:\n"
    "\u2022 Drawing note, label, Tolerance, description corrections, Revision correction\n"
    "\u2022 OMS(0251-XXXXX) typos or incorrect references\n"
    "\u2022 Balloon or callout corrections (intent unchanged)\n"
    "\u2022 PQC table typo or alignment fixes ONLY\n"
    "\u2022 Documentation issues found after Production release\n\n"
    "NOT allowed for 4.5:\n"
    "\u2022 Any physical or functional impact\n"
    "\u2022 Any correction from managed builds, NPI, or first-pass FAI\n"
    "\u2022 Any change impacting inspection or pass/fail logic\n\n"
    "---------------------------------------------------------------------\n"
    "TOLERANCE AND INSPECTION RULE (NON\u2011NEGOTIABLE)\n"
    "---------------------------------------------------------------------\n"
    "\u2022 Adding tolerances or GD&T ALWAYS impacts inspection and acceptance\n"
    "  \u2192 MUST NOT be classified as Document Correction\n"
    "\u2022 Removing tolerances is Document Correction ONLY if:\n"
    "  \u2013 removal is cleanup (duplicate/conflicting)\n"
    "  \u2013 inspection and acceptance criteria are unchanged\n"
    "\u2022 Any tolerance change that alters pass/fail outcomes\n"
    "  \u2192 MUST NOT be 4.5\n\n"
    "---------------------------------------------------------------------\n"
    "PRODUCT RELEASE \u2013 4.1\n"
    "---------------------------------------------------------------------\n"
...
def classify_reason_code_with_llm(problem: str, solution: str) -> tuple:
    ...
    Returns:
        (code_name, justification) where code_name is the display name
        (e.g. 'Design Correction') and justification is the LLM rationale.
