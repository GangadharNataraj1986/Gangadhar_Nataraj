...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QComboBox, QFileDialog, QHeaderView, QGraphicsDropShadowEffect, QFrame, QRadioButton, QButtonGroup,
 QGridLayout
)
...
EC_DECISION_FLOW = {
    ...
    # -------- Radio button entry points --------
    "Up Revision": "start",
    "Status Roll": "status_roll",
    "Production Release": "B3",
    "OBS / Inactivate": "B1",
    "Product Release": "PR",
    ...
    "start": {
        "question": "SMBoM Impacted?",
        "options": {"Yes": "obs_kit_uprev", "No": "part_status1"}
    },

    "obs_kit_uprev": {
        "question": "OBS or Inactive Kit / Piece Parts?",
        "options": {"Yes": "A1", "No": "A2"}
    },

    "part_status1": {
        "question": "Part Status?",
        "options": {"EVAL": "B2", "PROD": "B3"}
    },

    "status_roll": {
        "question": "Transition Type?",
        "options": {
            "EVAL → Production": "B3",
            "Concept → BOM List": "obs_kit_transition"
        }
    },

    "obs_kit_transition": {
        "question": "OBS Kit / Piece?",
        "options": {"Yes": "A1", "No": "A2"}
    },

    "production_release": {
        "question": "Production Release?",
        "options": {"Yes": "B3", "No": "obs_inactivate"}
    },

    "obs_inactivate": {
        "question": "OBS / Inactivate?",
        "options": {"Yes": "B1", "No": "review"}
    },

    "PR": {
        "question": "SmBOM Impacted?",
        "options": {"Yes": "A2", "No": "part_status2"}
    },
    ...
    # -------- End States --------
    "A1": {"result": "A1 – SMBoM Options as revised items and having CDW"},
    "A2": {"result": "A2 – SMBoM Options as revised items, and No CDW"},
    "B1": {"result": "B1 – No SMBoM Options as revised items and having CDW"},
    "B2": {"result": "B2 – No SMBoM Options as revised items, No CDW, and revised item status at Eval and/or moving to Eval or adding already released parts to Proto buckets"},
    "B3": {"result": "B3 – No SMBoM Option as revised items, No CDW, and revised Item status at Production and/or moving to Production"},
    "review": {"result": "Review Required – No EC category matched"}
}

# ===================== UI CLASS =====================

class ECCreationFormTab(QWidget):
    def __init__(self):
        super().__init__()
        self.flow = EC_DECISION_FLOW

        main_layout = QVBoxLayout(self)

        title = QLabel("EC Creation Form")
        title.setFont(QFont("Segoe UI", 14, QFont.Weight.DemiBold))
        ...
        for name in ["Up Revision", "Status Roll", "Production Release", "OBS / Inactivate", "Product Release"]:
            rb = QRadioButton(name)
            self.radio_group.addButton(rb)
            radio_layout.addWidget(rb)

        self.radio_group.buttonClicked.connect(self.rebuild_flow)
        ...
        # -------- Result Label --------
        self.result = QLabel("")
        self.result.setStyleSheet("color: green; font-weight: bold; padding-top: 10px")
    ...
    def rebuild_flow(self):
        ...
        start_node = self.flow.get(btn.text())
        self.process_node(start_node)

    def process_node(self, node_key):
        node = self.flow.get(node_key, {})

        if "result" in node:
            # ✅ Show end state in flow
            self.add_end_state(node["result"])
            # ✅ Also show in result label
            self.result.setText(f"EC Category: {node['result']}")
        else:
            self.add_question(node_key)

    def add_question(self, key):
        node = self.flow[key]

        row = QHBoxLayout()

        label = QLabel(node["question"])
        ...
        combo.currentTextChanged.connect(
            lambda text, c=combo, k=key: self.on_answer(text, c, k)
        )
    ...
    def add_end_state(self, text):
        row = QHBoxLayout()

        label = QLabel("EC Category:")
    ...
    def on_answer(self, text, combo, key):
        if text == "Select...":
            return

        next_node = self.flow[key]["options"][text]

        current_index = self.find_row_index(combo)
        while self.vbox.count() > current_index + 1:
    ...
    def find_row_index(self, widget):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.tabs.addTab(ECCreationFormTab(), "EC Creation Input Form") 
        self.tabs.setDocumentMode(True); self.tabs.setMovable(True)
    ...
    # self.tabs.addTab(PlaceholderTab("EC Creation Form"), "EC Creation Form")
