# ecr_kit_ui.py (Enhanced v7.4 – Where Used Import; Check Point+Validation; auto-check; row height tweaks)
import sys
import json
...
class ECRFrontPageTab(QWidget):
    ...
    CHECKLIST_DEF = [
        ...
        ("Interchangeability & Tags", ["Are the changes FFF compatible (per 03-3-10 Interchangeability Policy)? Provide system details.", "Provide interchangeability/tagging details as per CRP."]),
        ("Testing, Reports & Cost", ["Are test results/FDR available for all IFF impacted parts/critical parts?", "If the project relates to DCR (cost reduction), provide cost-saving details."])
    ]
    _QUESTIONS_TOTAL = sum(len(qs) for _, qs in CHECKLIST_DEF)
    assert len(CHECKLIST_DEF) == 10 and _QUESTIONS_TOTAL == 16
...
class OBSTable(QTableWidget):
    ...
    def _copy_selection_to_clipboard(self):
        sel=self.selectedRanges(); if not sel: return
        r=sel[0]; rows=[]
    ...
    def _paste_from_clipboard(self):
        text=QGuiApplication.clipboard().text(); if not text: return
        ...
        for r_offset,line in enumerate(lines):
            ...
            for c_offset,val in enumerate(parts):
                ...
                else: self.setItem(row,col,QTableWidgetItem(val))
...
class WhereUsedTab(QWidget):
    """Import 'Where Used' parents Excel and display as-is."""
    def __init__(self):
        super().__init__()
        outer=QVBoxLayout(self)
        title_row=QHBoxLayout()
        ...
        btn_import.setToolTip("Import Excel and display it as-is (extension mismatch warnings are ignored)")
    ...
    def import_where_used(self):
        try:
            path,_=QFileDialog.getOpenFileName(self, "Import 'Where Used' Parents", '', "Excel Files (*.xlsx *.xls);;All Files (*.*)")
            if not path: return
            import pandas as pd
            df=None; errors=[]
            try:
                if path.lower().endswith('.xls'): df=pd.read_excel(path, engine='xlrd')
                else: df=pd.read_excel(path, engine='openpyxl')
            except Exception as e:
                errors.append(str(e))
            if df is None:
                try:
                    if path.lower().endswith('.xls'): df=pd.read_excel(path, engine='openpyxl')
            ...
            if df is None:
                try: df=pd.read_excel(path)
                    ...
                    errors.append(str(e))
            if df is None: raise RuntimeError("Failed to open the Excel file.\n"+"\n".join(errors[-3:]))
