...
class WhereUsedTab(QWidget):
    """Import 'Where Used' parents. If the file is legacy/malformed (.xls or HTML-disguised),
    the app will silently open it in Excel (no prompts), SaveAs a modern .xlsx in the background,
    then import the converted file. Paste from clipboard is intentionally removed."""
    def __init__(self):
        super().__init__()
        outer=QVBoxLayout(self)
        title_row=QHBoxLayout()
        ...
        btn_import.setToolTip("Select Excel/CSV/HTML. Legacy or mismatched files are auto-converted to .xlsx using Excel, then imported.")
    ...
    def _convert_to_xlsx_via_excel(self, src_path: str) -> str | None:
        ...
        try:
            ...
            import win32com.client  # requires pywin32 installed on Windows
            ...
            excel.Visible = False
            excel.DisplayAlerts = False  # suppress extension mismatch prompts
            ...
            # Attempt to close Excel cleanly if we created it
            try:
                excel.Quit()
            except Exception:
                pass
    ...
    def import_where_used(self):
        try:
            ...
            # If still no df, try direct parsers
            if df is None:
                # Try clean .xlsx
                try:
            ...
            # Try HTML (only if dependencies available)
            if df is None and (lower.endswith('.htm') or lower.endswith('.html') or self._is_html_like(path)):
                try:
                    tables = pd.read_html(path)  # requires lxml/html5lib/bs4 if present
                    if tables:
                        df = tables[0]
                except Exception as e:
            ...
            msg = f"Imported {len(df)} rows from:\n{converted_path or path}"
            if converted_path:
