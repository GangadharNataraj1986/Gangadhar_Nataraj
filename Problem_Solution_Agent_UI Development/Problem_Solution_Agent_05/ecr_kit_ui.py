# ecr_kit_ui.py (Enhanced v5 per new requests)
import sys
import json
...
# ----------------- Global error handling (helps diagnose unexpected exits) -----------------
import traceback
...
def _handle_exception(exc_type, exc_value, exc_tb):
    try:
        ...
        # Show a non-blocking critical dialog so app does not silently exit
        m = QMessageBox()
        ...
        m.show()
        # Keep a reference so it doesn't get destroyed immediately
        # Attach to QApplication instance if available
...
class ECRFrontPageTab(QWidget):
    # Define categories with main and sub validations (sub lines can be highlighted)
    # Structure: (Category, [(text, highlight_bool), ...])
    CHECKLIST = [
        ("Project Association", [
            ("Does the ECR include a Project (PCR)?", False),
        ]),
        ("Safety & Compliance", [
            ("Provide PSER details if any safety incident occurred.", False),
            ("Is a PCN required and approved by ROW and CE?", True),
        ]),
        ("Part Release Status", [
            ("Are all BTPs and kits EVAL released?", False),
            ("If the parent part is in production, is the replacement part (BTP) production released?", True),
        ]),
        ("Design Analysis", [
            ("Is VA/VB analysis completed (for new designs/parts only)?", False),
            ("PACE / DASH parts addressed?", True),
        ]),
        ("Watchlist & Spares", [
            ("Are all new parts/designs MLO certified / parent/previous parts were MLO certified?", False),
            ("Do we have AGS approval for OBS parts (sparable) without replacement?", True),
        ]),
        ("Config Documents", [
            ("Is CCR available with change matrix details for new options/reference designator updates?", False),
        ]),
        ("ECR Strategies Identified", [
            ("Provide reason code, effective strategy, priority, and disposition.", False),
        ]),
        ("Multi BU Alignment", [
            ("If scope impacts multiple BUs/products, verify and confirm all affected BUs/products are listed in the project and approved.", False),
        ]),
        ("Interchangeability & Tags", [
            ("Are the changes FFF compatible (per 03-3-10 Interchangeability Policy)? Provide system details.", False),
        ]),
        ("Testing & Reports", [
            ("Are test results/FDR available for all IFF impacted parts/critical parts?", False),
        ]),
        ("Cost & Savings", [
            ("If the project relates to DCR (cost reduction), provide cost-saving details.", False),
        ]),
    ]
    ...
    def __init__(self):
        ...
        self.table = QTableWidget(len(self.CHECKLIST), 7)
        ...
        # Populate rows
        for row, (cat, sublist) in enumerate(self.CHECKLIST):
            # Sl.No non-editable
            ...
            # Validation as rich text using QLabel to support highlighted sub-lines
            html_lines = []
            for text, hl in sublist:
                if hl:
                    html_lines.append(f"<div style='background-color:#FFF59D;padding:2px 2px;'>{text}</div>")
                else:
            ...
            self.table.setItem(row, 6, QTableWidgetItem(""))   # Comments (last)
        ...
        try:
            ...
            header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)  # Completion
            header.setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)  # Category
            header.setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)           # Validation (rich text, stretch)
            header.setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)           # Action Owner
            header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents)  # Due Date
        ...
        self.progress_label = QLabel(f"Checklist Progress: 0 / {len(self.CHECKLIST)}")
        self.progress_bar = QProgressBar(); self.progress_bar.setRange(0, len(self.CHECKLIST))
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for r in range(t.rowCount()):
            ...
            row = {
                ...
                'validation_html': t.cellWidget(r, 3).text() if isinstance(t.cellWidget(r,3), QLabel) else '',
...
class OBSTable(QTableWidget):
    ...
    def apply_style(self):
        self.setStyleSheet("""
            ...
            QHeaderView::section { background-color:#F0F5FF; color:#C1272D; padding:6px; border:1px solid #D5E3F6; font-weight:600; }
        """)
    ...
    def _make_change_combo(self) -> QComboBox:
        ...
        combo.setStyleSheet("QComboBox { background: #FFFDE7; }")
        return combo
...
def run():
    ...
    # Keep the app robust during focus changes like Alt+Tab by relying on global excepthook
