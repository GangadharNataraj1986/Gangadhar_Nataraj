# ecr_kit_ui.py (Enhanced v7.3 – Check Point + Validation column; YES/NO/N/A + auto-check; fixed row height)
import sys
import json
...
class ECRFrontPageTab(QWidget):
    ROW_HEIGHT = 40  # maintain uniform row height to keep multiple rows visible
    ...
    def __init__(self):
        ...
        # Column order: Sl.No | Completion | Category | Check Point | Validation | Action Owner | Due Date | Comments
        self.table = QTableWidget(len(self.CHECKLIST_DEF), 8)
        self.table.setHorizontalHeaderLabels([
            "Sl.No", "Completion", "Category", "Check Point", "Validation", "Action Owner", "Due Date", "Comments"
        ])
        ...
        for row, (cat, qlist) in enumerate(self.CHECKLIST_DEF):
            ...
            # Check Point: multi-line label with the questions
            cp_text = "\n".join(qlist)
            cp_label = QLabel(cp_text)
            cp_label.setTextFormat(Qt.TextFormat.PlainText)
            cp_label.setWordWrap(True)
            self.table.setCellWidget(row, 3, cp_label)

            # Validation: YES/NO/N/A selector
            combo = QComboBox(); combo.addItems(['', 'YES', 'NO', 'N/A'])
            self.table.setCellWidget(row, 4, combo)
            # Auto-check completion if YES or N/A selected (untick for NO or blank)
            def _on_validation_change(_txt, r=row):
                sel = (_txt or '').strip().upper()
                w = self.table.cellWidget(r, 1)
                if isinstance(w, QWidget) and hasattr(w, '_chk'):
                    w._chk.setChecked(sel in ('YES', 'N/A'))
            combo.currentTextChanged.connect(_on_validation_change)

            # Editable cells for user typing
            self.table.setItem(row, 5, QTableWidgetItem(""))  # Action Owner
            self.table.setItem(row, 6, QTableWidgetItem(""))  # Due Date
            ...
            self.table.setCellWidget(row, 7, te)

        # Header sizing & column widths
        header = self.table.horizontalHeader()
        header.setStretchLastSection(False)
        header.setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)       # Check Point column stretches
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # Validation dropdown width
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.Interactive)   # Action Owner
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.Interactive)   # Due Date
        header.setSectionResizeMode(7, QHeaderView.ResizeMode.Stretch)       # Comments

        # Column widths per requirement
        self.table.setColumnWidth(5, _excel_width_to_px(self.table, 16))                        # Action Owner = 75 px
        self.table.setColumnWidth(6, _excel_width_to_px(self.table, 14))  # Due Date similar to earlier
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for r in range(t.rowCount()):
            ...
            # Capture check point text
            cp_widget = t.cellWidget(r, 3)
            cp_text = cp_widget.text() if isinstance(cp_widget, QLabel) else ''
            # Capture validation choice
            v_widget = t.cellWidget(r, 4)
            v_choice = v_widget.currentText() if isinstance(v_widget, QComboBox) else ''

            row = {
                'slno': t.item(r, 0).text() if t.item(r, 0) else str(r+1),
                'completion': completion,
                'category': t.item(r, 2).text() if t.item(r, 2) else '',
                'check_point_text': cp_text,
                'validation_choice': v_choice,
                'action_owner': t.item(r, 5).text() if t.item(r, 5) else '',
    ...
    def from_dict(self, data: Dict[str, Any]):
        ...
        for r in range(rows):
            ...
            # Restore validation drop-down if present
            vc = row.get('validation_choice', None)
            wv = t.cellWidget(r, 4)
            if vc is not None and isinstance(wv, QComboBox):
                idx = wv.findText(vc)
                wv.setCurrentIndex(idx if idx >= 0 else 0)
            ...
            def _ensure_item(col):
                ...
                if it is None:
                    ...
                    t.setItem(r, col, it)
                return it
            _ensure_item(5).setText(row.get('action_owner', '') or row.get('actioner_owner',''))
            _ensure_item(6).setText(row.get('due_date', ''))
