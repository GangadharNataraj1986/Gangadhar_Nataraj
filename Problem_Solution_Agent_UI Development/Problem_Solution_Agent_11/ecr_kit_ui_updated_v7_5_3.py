# ecr_kit_ui.py (Enhanced v7.5.3 – Where Used: HTML/CSV auto-detect + Paste; Checklist updates)
import sys
import json
...
class OBSPartsTab(QWidget):
    def __init__(self):
        ...
        title_row.addWidget(title); title_row.addStretch(1); title_row.addWidget(btn_template); title_row.addWidget(btn_upload); title_row.addWidget(delete_btn)
...
class WhereUsedTab(QWidget):
    """Import 'Where Used' parents Excel and display as-is, with HTML/CSV auto-detection and Paste support."""
    def __init__(self):
        super().__init__()
        outer=QVBoxLayout(self)
        title_row=QHBoxLayout()
        ...
        btn_import.setToolTip("If you see an Excel message about file extension mismatch, click YES to open. This tool reads the file as-is and displays it.")
        title_row.addWidget(title); title_row.addStretch(1); title_row.addWidget(btn_import); title_row.addWidget(btn_paste)
        outer.addLayout(title_row)
    ...
    def import_where_used(self):
        try:
            ...
            try:
                ...
                errors.append(f'Excel engines failed: {e}')
            # Try HTML disguised as Excel
            if df is None:
                try:
                    with open(path, 'rb') as f:
                        head = f.read(512).lstrip()
                    if head.startswith(b'<!') or head.lower().startswith(b'<html') or b'<table' in head.lower():
                        tables = pd.read_html(path)
                        if tables:
                            df = tables[0]
                except Exception as e:
                    errors.append(f'HTML parse failed: {e}')
            # Try CSV (comma then tab)
            if df is None:
                try:
                    df = pd.read_csv(path)
                except Exception as e:
            ...
            if df is None: raise RuntimeError("Failed to open the file. Tried Excel, HTML and CSV paths.\n"+"\n".join(errors[-4:]))

            cols=[str(c) if c is not None else '' for c in df.columns]
            self._load_dataframe(df, cols)
            QMessageBox.information(self,'Import Complete',f"Imported {len(df)} rows from:\n{path}")
        except Exception as e:
            QMessageBox.warning(self,'Import Error',str(e))

    def paste_from_clipboard(self):
        # Accepts a grid copied from Excel/Browser and pastes into the table
        text = QGuiApplication.clipboard().text()
        if not text:
            QMessageBox.information(self,'Clipboard','Clipboard is empty or not text.'); return
        lines=[ln for ln in text.splitlines() if ln.strip()]
        if not lines:
            QMessageBox.information(self,'Clipboard','No tabular text detected.'); return
        rows=[ [cell.strip() for cell in line.split('\t')] for line in lines ]
        max_cols=max(len(r) for r in rows)
        self.table.clear(); self.table.setRowCount(len(rows)); self.table.setColumnCount(max_cols)
        # First row as header if looks like header
        header_guess = rows[0]
        self.table.setHorizontalHeaderLabels([str(c) for c in header_guess])
        start_data_row=1
        # If the first row is not header-like (contains only numbers), fallback to generic column names
        if all((not col) or col.isdigit() for col in header_guess):
            self.table.setHorizontalHeaderLabels([f'Col {i+1}' for i in range(max_cols)])
            start_data_row=0
        for r in range(start_data_row, len(rows)):
            for c in range(len(rows[r])):
                self.table.setItem(r-start_data_row, c, QTableWidgetItem(rows[r][c]))
        header=self.table.horizontalHeader()
        for i in range(max_cols):
            if i==max_cols-1: header.setSectionResizeMode(i,QHeaderView.ResizeMode.Stretch)
            else: header.setSectionResizeMode(i,QHeaderView.ResizeMode.ResizeToContents)
        QMessageBox.information(self,'Paste Complete', f'Pasted {len(rows)-start_data_row} rows from clipboard.')

    def _load_dataframe(self, df, cols):
        import pandas as pd
        self.table.clear(); self.table.setColumnCount(len(cols)); self.table.setHorizontalHeaderLabels(cols); self.table.setRowCount(len(df))
        for r in range(len(df)):
            for c in range(len(cols)):
                val=df.iloc[r,c]
                txt='' if (val is None or (hasattr(pd,'isna') and pd.isna(val))) else str(val)
                self.table.setItem(r,c,QTableWidgetItem(txt))
        header=self.table.horizontalHeader()
        for i in range(len(cols)):
            if i==len(cols)-1: header.setSectionResizeMode(i,QHeaderView.ResizeMode.Stretch)
            else: header.setSectionResizeMode(i,QHeaderView.ResizeMode.ResizeToContents)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.whereused_tab = WhereUsedTab()
