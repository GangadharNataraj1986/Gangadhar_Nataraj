...
class WURemovedBOMItemsTab(QWidget):
    ...
    def import_excel(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Import WU of Removed BOM items", '', "Excel Files (*.xlsx)")
        if not path:
            return
        import pandas as pd
        df = pd.read_excel(path, engine='openpyxl')
    ...
    def _build_obs_change_map(self):
        ...
        t = self.obs_provider.table
        for r in range(t.rowCount()):
            obs_it = t.item(r, 1)  # OBS Parts
            chg_w = t.cellWidget(r, 2)  # Change
            obs = obs_it.text().strip().upper() if obs_it else ''
            if obs:
                mapping[obs] = chg_w.currentText() if chg_w else ''
        return mapping
