...
class WhereUsedTabV2(WhereUsedTab):
    ...
    def __init__(self, obs_provider=None):
        ...
        row2 = [self.v2_btn_move, self.v2_btn_append, self.v2_btn_delete, self.v2_btn_export, self.v2_btn_reset]
...
class WURemovedBOMItemsTab(QWidget):
    ...
    def _build_obs_change_map(self):
        ...
        for r in range(t.rowCount()):
            ...
            obs = (obs_it.text() if obs_it else '').strip()
            if obs:
                mapping[obs.upper()] = chg_w.currentText() if chg_w else ''
        return mapping
    ...
    def import_excel(self):
        ...
        cols = list(df.columns)
        cols.insert(2, 'Orphan Child')
        df.insert(2, 'Orphan Child', '')
        ...
        for r in range(len(df)):
            chk = QCheckBox(); cont = QWidget(); h = QHBoxLayout(cont)
            h.setContentsMargins(0,0,0,0); h.setAlignment(Qt.AlignmentFlag.AlignCenter)
            h.addWidget(chk)
            self.table.setCellWidget(r, 0, cont)
            for c, col in enumerate(cols):
                val = '' if pd.isna(df.iloc[r, c]) else str(df.iloc[r, c])
    ...
    def perform_orphan_analysis(self):
        ...
        for r in range(self.table.rowCount()):
            ...
            if not it:
                continue
            key = it.text().lstrip().strip().upper()
            val = obs_map.get(key, '')
            if val:
                self.table.setItem(r, ocol, QTableWidgetItem(val))
