# ecr_kit_ui.py (Enhanced v4 per user requests)
import sys
import json
...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QProgressBar, QSizePolicy, QScrollArea, QToolBar, QFileDialog,
...
class ECRFrontPageTab(QWidget):
    ...
    def __init__(self):
        ...
        # Column order UPDATED: Sl.No | Completion | Category | Validation | Action Owner | Due Date | Comments (moved last)
        table = QTableWidget(len(self.CHECKLIST_ROWS), 7)
        table.setHorizontalHeaderLabels(["Sl.No", "Completion", "Category", "Validation", "Action Owner", "Due Date", "Comments"])
        ...
        for row, (cat, desc) in enumerate(self.CHECKLIST_ROWS):
            ...
            # Editable text cells
            table.setItem(row, 4, QTableWidgetItem(""))   # Action Owner
            table.setItem(row, 5, QTableWidgetItem(""))   # Due Date
            table.setItem(row, 6, QTableWidgetItem(""))   # Comments (moved last)
        ...
        try:
            ...
            header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)  # Completion checkbox
            header.setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)  # Category — fit text
            header.setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)  # Validation — fit text
            ...
            header.setSectionResizeMode(6, QHeaderView.ResizeMode.Stretch)           # Comments (last)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for r in range(t.rowCount()):
            ...
            row = {
                ...
                'validation': t.item(r, 3).text() if t.item(r, 3) else '',
...
class OBSTable(QTableWidget):
    def __init__(self, parent=None):
        ...
        try:
            ...
            header.setSectionResizeMode(1, QHeaderView.ResizeMode.Interactive)  # we will set exact width ~ Excel 14 chars
    ...
    def _apply_excel_widths(self, chars: int = 14):
        # Approximate Excel character width using '0' character in current font metrics
        fm = self.fontMetrics()
        px = fm.horizontalAdvance('0' * chars) + 16  # padding
        for col in (1, 2, 3):
            try:
    ...
    def _init_rows(self, start, end):
        for r in range(start, end):
            ...
            # Change (default Obsolete selectable via dropdown)
    ...
    # Allow Copy/Paste similar to Excel including multi-cell ranges
    ...
    def _copy_selection_to_clipboard(self):
        ...
        for i in range(r.rowCount()):
            ...
            for j in range(r.columnCount()):
                ...
                txt = ''
                if col_j == 2:  # Change column via combo
    ...
    def _paste_from_clipboard(self):
        ...
        for r_offset, line in enumerate(lines):
            ...
            for c_offset, val in enumerate(parts):
                ...
                if col == 0:
                    continue  # don't paste into Select column
                if col == 2:
                    ...
                    if isinstance(w, QComboBox):
                        ...
                        if idx >= 0:
                            ...
                            # If unexpected text, default stays as-is
                            pass
                else:
                    self.setItem(row, col, QTableWidgetItem(val))
        # after paste, re-apply widths (if font changed) and ensure visibility
        self._apply_excel_widths()
...
class OBSPartsTab(QWidget):
    def __init__(self):
        ...
        title.setStyleSheet("color:#C1272D;")  # red font for heading
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        act_load.triggered.connect(self.load_data_dialog)
        tb.addAction(act_load)

        # Auto load last data file if present
    ...
    def load_data_dialog(self):
        if DATA_FILE.exists():
            # Basic loader just reads the default file (kept simple per requirement)
            try:
                data = json.loads(DATA_FILE.read_text(encoding='utf-8'))
                self.apply_data(data)
