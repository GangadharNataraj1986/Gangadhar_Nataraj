...
class WURemovedBOMItemsTab(QWidget):
    ...
    def perform_orphan_analysis(self):
        ...
        # Helpers to normalize WU Level values
        def is_zero(v):
        ...
        # Step 1 & 2: Focused block orphan analysis
        r = 0
        while r < row_count:
            wu = self.table.item(r, wucol).text() if self.table.item(r, wucol) else ''

            if is_zero(wu):  # Child row (WU Level = 0)
                child_row = r
                ...
                # Collect valid parents (WU Level = 1)
                parents = []
        ...
        # Step 3: Propagate Orphan1 to repeated parts
        for r in range(row_count):
            it = self.table.item(r, pcol)
