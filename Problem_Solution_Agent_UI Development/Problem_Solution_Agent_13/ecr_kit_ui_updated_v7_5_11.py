...
class WhereUsedTabV2(WhereUsedTab):
    ...
    def __init__(self, obs_provider=None):
        ...
        self.v2_btn_sel_9024.setStyleSheet(btn_style('#4F86F7','#2E6FE3','#275ECB'))   # blue
        self.v2_btn_sel_opt.setStyleSheet(btn_style('#2BB673','#22995F','#1F8452'))     # green
        self.v2_btn_sel_esw.setStyleSheet(btn_style('#8E5AD7','#7B48C6','#6A3BB5'))     # purple
        self.v2_btn_move.setStyleSheet(btn_style('#26C6DA','#14A7BE','#0F93AA'))        # cyan
        self.v2_btn_append.setStyleSheet(btn_style('#EC6AA0','#D6538C','#C04378'))      # pink
        self.v2_btn_refresh.setStyleSheet(btn_style('#66BB6A','#4EA85A','#3F8F4A'))     # fresh green
        self.v2_btn_delete.setStyleSheet(btn_style('#EF5350','#D84343','#C43838'))      # red
        self.v2_btn_reset.setStyleSheet(btn_style('#90A4AE','#7C919B','#6A7E87'))       # gray
        self.v2_btn_export.setStyleSheet(btn_style('#FFA726','#FB8C00','#EF6C00'))      # orange

        # Arrange in 2 rows
        row1 = [self.v2_btn_sel_9024, self.v2_btn_sel_opt, self.v2_btn_sel_esw, self.v2_btn_sel_above_cfg, self.v2_btn_refresh]
        row2 = [self.v2_btn_move, self.v2_btn_append, self.v2_btn_delete, self.v2_btn_reset, self.v2_btn_export]
    ...
    def _v2_select_above_config_block(self):
        ...
        while r < rows:
            wu_val = (self.table.item(r, wu_col).text() if self.table.item(r, wu_col) else '').strip()

            if wu_val == '0':
                block_start = r
                block_end = rows

                # find end of this WU block
                for i in range(r + 1, rows):
                    nxt = (self.table.item(i, wu_col).text() if self.table.item(i, wu_col) else '').strip()
                    if nxt == '0':
                        block_end = i
                        break
                ...
                # find config row by indentation
                ...
                # select children deeper than config
