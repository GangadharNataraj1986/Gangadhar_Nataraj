...
class WhereUsedTabV2(WhereUsedTab):
    ...
    def _v2_select_above_config_block(self):
        ...
        for r in range(rows):
            part = (self.table.item(r, part_col).text() if self.table.item(r, part_col) else '').strip()
            if part[:4] not in prefixes:
                continue
            start = 0
            for i in range(r, -1, -1):
                val = (self.table.item(i, wu_col).text() if self.table.item(i, wu_col) else '').strip()
                if val == '0':
                    start = i
                    break
