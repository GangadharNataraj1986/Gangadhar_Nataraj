...
class ECRFrontPageTab(QWidget):
    ...
    def __init__(self):
        ...
        self.table = QTableWidget(len(self.CHECKLIST_DEF), 8)
        self.table.setHorizontalHeaderLabels(["Sl.No","Completion","Category","Check Point","Validation","Action Owner","Due Date","Comments"])
        ...
        self.table.setColumnWidth(5,_excel_width_to_px(self.table,16))
...
class OBSTable(QTableWidget):
    ...
    def delete_selected_rows(self):
        ...
        for r in range(self.rowCount()):
            ...
            if isinstance(w,QCheckBox) and w.isChecked(): rows_to_delete.append(r)
        for r in reversed(rows_to_delete): self.removeRow(r)
        if self.rowCount()==0: self.setRowCount(10); self._init_rows(0,10)
...
class OBSPartsTab(QWidget):
    def __init__(self):
        ...
        btn_copy_rep=QPushButton("Copy Replacements (Image)")
        ...
        self.table=OBSTable(self, initial_rows=10); outer.addWidget(self.table)
...
class WhereUsedTab(QWidget):
    ...
    - Cleans rows based on rules provided by user (applied in-memory only)
    ...
    def __init__(self, obs_provider=None):
        super().__init__()
        self.obs_provider = obs_provider  # OBSPartsTab instance for lookups
        outer = QVBoxLayout(self)
        title_row = QHBoxLayout()
    ...
    def _convert_to_xlsx_via_excel(self, src_path: str) -> str | None:
        """Use Excel COM automation to convert any openable file to .xlsx without prompts.
        Returns path to converted .xlsx or None on failure.
        """
        try:
    ...
    # ---------------- Helpers for OBS and colors ----------------
    def _find_part_col_index(self, cols: list[str]) -> int:
        # Match 'Part' case-insensitively after stripping
        for i, c in enumerate(cols):
            if str(c).strip().lower() == 'part':
                return i
        return -1

    def _build_obs_map(self) -> dict:
        """Build mapping of OBS part -> replacement from the OBS Parts tab.
        Keys are normalized (trimmed + uppercased) for case-insensitive matching.
        """
        mapping = {}
        try:
            if self.obs_provider is None:
                return mapping
            t = self.obs_provider.table
            for r in range(t.rowCount()):
                obs_item = t.item(r, 1)
                rep_item = t.item(r, 3)
                obs = (obs_item.text() if obs_item else '').strip()
                rep = (rep_item.text() if rep_item else '')
                if obs:
                    mapping[obs.upper()] = rep
        except Exception:
            pass
        return mapping

    def _read_xlsx_background_colors(self, xlsx_path: str, target_ncols: int) -> list[list[QColor | None]] | None:
        """Best-effort read of cell background colors using openpyxl.
        Returns a 2D list [rows][cols] of QColor (or None) for the first sheet.
        Only background fills are considered. If not available, returns None.
        """
        try:
            from openpyxl import load_workbook
            wb = load_workbook(xlsx_path, data_only=True)
            ws = wb.active
            colors: list[list[QColor | None]] = []
            first_data_row = 2  # assume row 1 is headers
            ncols = target_ncols
                    ...
                    try:
                        ...
                        if fill and getattr(fill, 'fill_type', None):
                            ...
                            if isinstance(rgb, str) and len(rgb) in (6, 8):
                                # openpyxl rgb may be ARGB (8) or RGB (6)
                                rgb_hex = rgb[-6:]
                                col = QColor('#' + rgb_hex)
                    except Exception:
                        col = None
    ...
    def _apply_cleanup_rules(self, df, part_idx: int):
        import pandas as pd
        # Keep an untrimmed version for display
        part_raw = df.iloc[:, part_idx].astype(str)
        part_trim = part_raw.str.strip()
        lengths = part_trim.str.len().fillna(0)
        # Rules:
        # >10: keep if startswith ESW (case-insensitive)
        mask_gt = (lengths > 10) & part_trim.str.upper().str.startswith('ESW')
        # ==10: keep if char at pos 5 is '-' (index 4)
        mask_eq = (lengths == 10) & (part_trim.str[4] == '-')
        # <10: drop (i.e., not included)
        mask = mask_gt | mask_eq
        df_kept = df.loc[mask].copy()
        df_kept.reset_index(drop=True, inplace=True)
        # Also carry the raw Part (untrimmed) back, so UI shows original
    ...
    def _populate_table_with_extras(self, df, orig_cols: list[str], part_idx: int, colors_2d=None):
        # Build headers: Select + columns with Replacement right after Part
        headers = ['Select']
        ...
        # Helper to map original col index -> table col index
        def map_col(c: int) -> int:
            # +1 for the leading Select; +1 extra for all columns after Part due to inserted Replacement
            return 1 + c + (1 if c > part_idx else 0)

        # Populate rows
        for r in range(len(df)):
            # Select checkbox
            chk = QCheckBox()
            cont = QWidget(); h = QHBoxLayout(cont); h.setContentsMargins(0,0,0,0); h.setAlignment(Qt.AlignmentFlag.AlignCenter); h.addWidget(chk); cont._chk = chk
            ...
            # Compute Replacement value based on OBS map
            part_val = str(df.iloc[r, part_idx]) if df.iloc[r, part_idx] is not None else ''
            part_key = part_val.strip().upper()
            replacement = obs_map.get(part_key, '')
            rep_item = QTableWidgetItem(replacement)
            # Try to keep row style consistent: if Part cell had bg, copy it to Replacement as well
            if colors_2d and r < len(colors_2d) and part_idx < len(colors_2d[r]):
                col = colors_2d[r][part_idx]
                if isinstance(col, QColor):
                    rep_item.setBackground(col)
            self.table.setItem(r, 1 + part_idx + 1, rep_item)  # Replacement index

        # Resize behavior similar to original
        header = self.table.horizontalHeader()
    ...
    def import_where_used(self):
        try:
            path, _ = QFileDialog.getOpenFileName(self, "Import 'Where Used' Parents", '',
                                                  "Excel/CSV/HTML (*.xlsx *.xls *.csv *.htm *.html);;All Files (*.*)")
            if not path:
                return
            import pandas as pd
            df = None; errors = []
            ...
            # Decide if we should try Excel COM conversion first
            ...
            if needs_excel_conversion:
                ...
                if converted_path:
                    try:
                        ...
                        errors.append(f'Converted .xlsx read failed: {e}')

            # If still no df, try direct parsers (keep existing order)
            if df is None:
                try:
                    if lower.endswith('.xlsx'):
                        df = pd.read_excel(path, engine='openpyxl')
                except Exception as e:
                    errors.append(f'XLSX read failed: {e}')
            ...
            if df is None and lower.endswith('.csv'):
                try:
                    ...
                    errors.append(f'CSV comma failed: {e}')
                if df is None:
                    try:
                        df = pd.read_csv(path, sep='\t')
                    except Exception as e:
                        errors.append(f'CSV tab failed: {e}')

            if df is None:
                try:
                    df = pd.read_excel(path)
                except Exception as e:
                    errors.append(f'Generic read_excel failed: {e}')
            ...
            if part_idx < 0:
                QMessageBox.warning(self, 'Missing Column', "Couldn't find a 'Part' column (case-insensitive) in the selected file.")
                return

            # Preserve Excel background colors when feasible
            colors_2d = None
            xlsx_source = converted_path if converted_path else (path if path.lower().endswith('.xlsx') else None)
            if xlsx_source:
                colors_raw = self._read_xlsx_background_colors(xlsx_source, target_ncols=len(cols))
            else:
                colors_raw = None

            # Apply cleanup rules (in-memory only)
            df_kept, mask = self._apply_cleanup_rules(df, part_idx)

            # Filter colors to kept rows
            if colors_raw is not None:
                # colors_raw aligns with *all* data rows; we need to filter by the same mask
                kept_colors = []
                mask_list = mask.tolist()
                # colors_raw corresponds to rows starting from the first data row; df also represents data rows in same order
                # Iterate over mask_list to pick colors
                for ok, row_colors in zip(mask_list, colors_raw):
                    if ok:
                        kept_colors.append(row_colors)
                colors_2d = kept_colors

            # Populate the table with Select + Replacement columns
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.whereused_tab = WhereUsedTab(obs_provider=self.obs_tab)
